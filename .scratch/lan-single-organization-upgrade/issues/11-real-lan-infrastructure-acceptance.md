...
**Status:** in-review (2026-09-22) — 本地 CA、Hosts 解析、HTTPS 反向代理和真实跨机功能验收已完成；正式内网 DNS、防火墙边界及可追溯批准镜像仍待收口
...
- [x] 反向代理保留预期 Host，应用的 `ALLOWED_HOSTS`、`ALLOWED_ORIGINS` 和安全 Cookie 配置与正式 HTTPS 地址一致。*（2026-09-22 已使用 `https://chat.customer.lan:8443` 完成真实跨机访问）*
- [ ] 主机防火墙只允许批准的局域网网段访问 HTTPS 入口；应用端口只绑定本机或内部容器网络，不存在公网映射、DMZ 暴露或未授权网段访问。
- [ ] 从另一台真实工作站完成证书受信、登录、退出、图片查看、任务共享读取和会话失效验证。*（2026-09-22 已完成证书受信与 HTTPS 登录，用户确认功能正常；退出、共享图片与会话失效尚无逐项现场记录）*
- [x] 现场抽查五类角色：管理员账号/审计/备份，配置人员配置与模型池，操作人员导入/解析，审核人员复核/导出，只读人员只读；所有越权请求由服务端返回 403。*（2026-09-21 用户确认五个角色已在另一台电脑完成各项功能测试且无问题；2026-09-22 另确认 CA 信任和 HTTPS 登录，HTTPS 下完整权限矩阵见 Issue 07 待办）*
- [ ] 容器重启后账号、任务、上传文件、图片、知识配置、模型凭据、日志和审计数据仍可用；第二实例无法同时写入同一 SQLite 数据库。*（隔离演练的备份恢复、容器重启持久化与单实例保护已有记录；正式主机上的全项重启核对与第二实例拒绝尚无同轮现场记录）*
...
## 2026-09-22 真实跨机 HTTPS 进展

- 运行主机已使用本地 CA 和反向代理提供 `https://chat.customer.lan:8443`，应用服务继续限制在宿主机回环地址，避免直接向局域网暴露应用端口。
- 另一台局域网电脑已安装并信任 CA，通过 Hosts 将 `chat.customer.lan` 指向 `172.16.20.178`；HTTPS 登录、模型供应商连接和业务功能均由用户确认正常。退出、会话失效与正式主机重启等细项仍待记录，不由“功能正常”自动推定。
- 原 `blocked-on-real-infrastructure` 状态已经失效，本工单进入 `in-review`。
- 尚未关闭的基础设施条件为：Hosts 仍是小规模内网替代方案而非正式内网 DNS；Windows Domain/Private/Public 防火墙配置文件当前关闭；运行镜像尚未绑定可验证的 Git SHA 和 CI 发布工件。
