...
**Status:** in-progress (2026-09-18) — 真实模型导入/解析/查看/导出与备份恢复已通过；复核、只读抽查、CI 镜像升级回滚待办

- [x] 管理员或配置人员通过正式界面配置真实模型供应商凭据，完成视觉与文本模型验证；凭据不会出现在前端响应、日志、审计或验收附件中。
- [x] 操作人员从第二台局域网工作站导入一份脱敏但结构真实的 Excel，创建任务并成功完成至少一条包含模型调用的解析。
- [ ] 审核人员查看待复核记录、提交复核结论并导出审核结果；导出文件包含预期原始字段、解析字段、状态和复核信息。*（导出已通过：23 列含原始字段/解析字段/状态/复核列；真实复核未执行，保留待办）*
- [ ] 只读人员可以查看共享任务和结果，但无法修改、复核、导出、配置模型或管理账号。
- [ ] 审计界面可通过关联 ID 追溯模型配置、导入、解析、复核、导出和权限拒绝，不包含密码、会话、API Key 或加密密钥。*（真实运行已产生模型配置/导入/解析/导出审计；关联 ID 追溯与权限拒绝抽查未在真实跨机流程执行，保留待办）*
...
- 导出 4 次成功，最新导出 23 列 × 10 行并与任务对应；模型池基线收敛后 `/api/ready` 通过，桩模型/测试供应商已停用并移出路由池。
- 备份与独立恢复：`restore:verify` 5/5 通过（数据库 v17、11 个文件引用、知识目录哈希、53 个模型凭据可解密）；容器重启后任务、记录、图片与审计不变。
- 仍未执行：真实复核、只读/越权抽查、主机级重启、正式 HTTPS、CI 工件升级回滚（工单 03）。
