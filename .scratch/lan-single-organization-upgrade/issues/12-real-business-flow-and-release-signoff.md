...
**Status:** in-progress (2026-09-22) — 真实模型业务闭环、五角色跨机功能抽查、跨机 HTTPS 登录和备份恢复已通过；HTTPS 下完整权限矩阵、CI 批准镜像升级回滚与最终发布签字仍待完成
...
- 第 3、4、5 项的真实复核、只读抽查和权限验证已有记录，跨机 HTTPS 登录也已确认；当前第 7、9 项继续等待 CI 批准镜像升级回滚和最终发布签字，HTTPS 下完整五角色矩阵另见 Issue 07。
...
- 另一台局域网电脑已信任本地 CA，通过 `https://chat.customer.lan:8443` 登录并使用系统；用户确认真实模型供应商连接及全部业务功能正常。
- 本结果确认 HTTPS 入口、第二工作站和真实模型可用性；不替代 HTTPS 下完整权限矩阵或 CI 批准镜像的升级回滚证据。
- 当前运行镜像仍使用开发构建元数据，故最终发布签字继续保持未完成。
