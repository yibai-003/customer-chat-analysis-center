...
- 正式模型验收由“单条模型调用”升级为同一任务的 20–50 条整批解析，并逐条检查模型输出、解析状态、复核状态和导出结果；导出文件必须包含原始字段和允许导出的 AI 字段，且不得包含解析状态、复核状态和复核备注。
- 临时角色账号改为每次随机密码和唯一用户名，验收结束全部停用；临时模型和无引用临时供应商在结束时删除，默认模型（含原本无默认的状态）与模型池设置恢复到验收前状态。
- 修复导出服务曾主动追加 `解析状态`、`复核状态`、`复核备注` 的回归，并加入“禁止写入这三列”的回归测试。
- 多板块导出仍按板块读取对应结果；复核状态和复核备注只在系统界面与数据库中保留，不进入业务导出文件。
