...
同一数据库启动第二个实例会失败并提示保护端口被占用。Windows 下错误信息会附带占用进程名与 PID，例如：

```text
该数据库已有服务运行，或本机保护端口 49719 被占用。占用进程：ROGLiveService.exe（PID 5816）。请确认它是另一个数据库实例还是无关软件。未修改数据库。
```

确认与处置：

1. 若占用进程是本项目的 node 进程，说明已有实例在运行；先停止旧实例（`Stop-Process -Id <PID>`）或继续使用旧实例，不要并行启动。
2. 若占用进程是无关软件，可手动核对端口归属：
   ```powershell
   netstat -ano | findstr :49719
   tasklist /FI "PID eq 5816"
   ```
   该端口由数据库路径哈希决定，无法换端口规避；请结束无关软件的监听、更换其端口，或确认该软件允许共存后调整本机的端口占用（不修改本项目锁逻辑）。
3. 不要在服务未停止时删除数据库或绕过保护强行启动。
