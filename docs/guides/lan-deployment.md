...
# 填写 APP_IMAGE、ENCRYPTION_KEY、SESSION_SECRET，确认 DEPLOY_DATA_DIR / DEPLOY_KNOWLEDGE_DIR
...
- **端口**：应用只绑定 `127.0.0.1:${APP_PORT}`，由内网反向代理 / TLS 终止器对外提供服务；不要直接把应用端口暴露到局域网或公网。
...
- **密钥**：`ENCRYPTION_KEY` 与 `SESSION_SECRET` 只存在于 `deploy/.env`（不提交、不打入镜像）。若不提供 `ENCRYPTION_KEY`，应用会在 `DATA_DIR/.secrets/` 生成托管密钥文件，必须单独备份。
...
- 以专用低权限账户运行；`deploy/.env.example` 中的密钥要求同样适用（`ENCRYPTION_KEY`、`SESSION_SECRET`）。
