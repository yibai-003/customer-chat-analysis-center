...
export class ModelBudgetExceeded extends Error {
  constructor() { super("字段模型请求已达到总预算（最多 6 次请求、5 分钟），请检查配置后重试"); }
}
...
export async function withModelBudget<T>(work: () => Promise<T>, options: { signal?: AbortSignal; requests?: number; timeoutMs?: number } = {}) {
  ...
  const timer = setTimeout(() => deadline.abort(new ModelBudgetExceeded()), timeoutMs);
  let used = 0;
  const budget: ModelBudget = {
    signal,
    check: () => { signal.throwIfAborted(); if (used >= requests) throw new ModelBudgetExceeded(); },
