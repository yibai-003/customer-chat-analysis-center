...
export function revertReceptionIssueRowExport(db: any) {
  ...
  const settings = JSON.parse(current.export_settings_json) as { rowMode?: string };
  if (settings.rowMode === "screenshot_records") return;
  ...
  const exportSettings = JSON.stringify({ ...settings, rowMode: "screenshot_records" });
  ...
  if (db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='audit_events'").get()) {
    ...
    `).run(
      ...
      JSON.stringify({
        ...
        upgrade: "revert-reception-issue-row-export",
      }),
      timestamp,
    );
  }
