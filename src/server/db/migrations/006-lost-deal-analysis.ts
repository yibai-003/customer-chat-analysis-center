...
function ensureItem(
  ...
  const values = {
    ...
    适用条件: condition,
    排除条件: excluded,
    ...(baseId === CUSTOMER_BASE_ID
      ? { 示例表达: `客户明确提到${name}相关顾虑` }
      : {
        改进方向: `针对${name}补充确认并给出明确回应`,
        示例话术: `关于${name}，我先确认您的具体情况，再给出对应方案。`,
      }),
  };
  const existing = db.prepare("SELECT values_json FROM knowledge_items WHERE id = ?").get(id) as
    | { values_json: string }
    | undefined;
  ...
  const searchText = Object.values(mergedValues).join(" ");
  db.prepare(`
    ...
    ON CONFLICT(id) DO UPDATE SET
      ...
      updated_at = excluded.updated_at
  `).run(id, baseId, pathKey, JSON.stringify(mergedValues), searchText, index + 1, timestamp, timestamp);
...
export function applyLostDealAnalysisConfiguration(db: any) {
  ...
  ensureBase(db, CUSTOMER_BASE_ID, "客户未成交原因库", [
    ...
    { name: "示例表达", roles: ["positive_example", "search"] },
  ], timestamp);
  ensureBase(db, SERVICE_BASE_ID, "客服促单问题库", [
    ...
    { name: "适用条件", roles: ["positive_example", "search"] },
    { name: "排除条件", roles: ["negative_example", "search"] },
    { name: "改进方向", roles: ["description", "search"] },
    { name: "示例话术", roles: ["positive_example", "search"] },
  ], timestamp);
