...
const preSaleRules = [
  ...
  ["PRE_PASSIVE_SERVICE", "服务消极D级", "客服明确拒绝合理接待、推诿或消极终止服务。", 0, true, 1, "承接客户诉求并明确下一步处理动作，避免推诿。"],
  ...
  ["PRE_UNRESOLVED", "问题未解决D级", "客服具备处理条件却未提供有效答案或方案，导致核心问题明确未解决。", 0, true, 1, "围绕核心诉求给出明确答案、方案和下一步。"],
  ...
  ["PRE_CHALLENGE_CUSTOMER", "反问/质疑顾客D级", "客服使用带责备、贬低或讽刺含义的反问质疑客户。", 0, true, 1, "改用中性确认句核实信息，避免责备式反问。"],
  ["PRE_NO_EMPATHY", "未有效安抚客户情绪D级", "客户明确表现负面情绪，客服未作任何安抚或正向承接。", 0, true, 1, "先回应客户情绪，再说明解决方案和处理进度。"],
  ...
  ["PRE_BAD_ATTITUDE", "态度差D级", "客服辱骂、讥讽、争吵、明显不耐烦或使用争议符号表达。", 0, true, 1, "保持专业克制，禁止讥讽、争吵和情绪化回复。"],
  ["PRE_CROSS_PLATFORM", "跨平台引导", "客服引导客户前往其他平台购买。", 0, true, 1, "禁止引导客户跨平台购买，按当前平台规范提供服务。"],
...
const afterSaleRules = [
  ["POST_NO_REPLY", "漏回复", "客户提出明确售后诉求或补充信息后，人工客服没有任何有效回应。", 1, "有效承接售后诉求并说明下一步处理动作。"],
  ["POST_NO_REGISTRATION", "漏登记", "规则要求登记且后台记录明确显示未登记。", 1, "按售后要求完成登记并核对记录。"],
  ["POST_NO_NOTE", "漏备注", "规则要求备注且后台记录明确显示未备注。", 1, "完成订单或售后单备注并核对内容。"],
  ["POST_NO_RESEND", "漏补发", "已确认需要补发，后台记录明确显示未创建或未完成补发。", 1, "确认补发后及时创建记录并跟进结果。"],
  ["POST_REGISTRATION_ERROR", "登记错误", "登记内容与聊天或订单中的商品、数量、金额、地址、原因等明确信息不一致。", 1, "登记前逐项核对订单和客户确认信息。"],
  ["POST_CHAT_CAUSED_COMPLAINT", "因聊天引发投诉或差评", "投诉或差评由本次客服态度或处理行为直接引发。", 1, "及时安抚并纠正不当沟通，避免服务问题升级。"],
  ["POST_REFUND_COMPLAINT_ERROR", "退款或投诉处理错误", "退款或投诉处理发生明确错误并造成实际损失，且未及时挽回。", 1, "核对金额与处理路径，发现错误后立即补救。"],
  ["POST_REFUND_REJECT_NO_EVIDENCE", "拒绝退款但未按要求核实取证", "规则要求核实、联系快递或上传凭证，记录明确显示客服未完成。", 1, "拒绝退款前按规则完成核实和取证。"],
  ["POST_BAD_PROCESS_OR_PRIVACY", "聊天处理错误、信息泄露或劣质服务", "客服提供明确错误信息、泄露敏感信息或存在有证据的劣质服务。", 1, "核对处理信息并严格保护客户隐私。"],
  ["POST_ATTITUDE", "态度问题", "客服辱骂、威胁、嘲讽、人身攻击或与客户争吵。", 5, "保持专业克制，禁止辱骂、威胁和争吵。"],
  ["POST_COLD_DELAY", "冷漠、不耐烦、敷衍或拖延", "客服无关敷衍、消极回避，或具备处理条件时故意拖延。", 1, "说明核实原因和预计进度，持续推进问题处理。"],
  ["POST_ABANDON_COMPLAINT", "买家差评、投诉或工商威胁时放任客户", "客户明确威胁投诉，客服未作安抚、解释、协商或解决尝试。", 1, "面对投诉风险应主动安抚并提出可执行方案。"],
  ["POST_SOP_VIOLATION", "未按照售后SOP解决问题且态度敷衍", "已提供适用SOP，客服处理明确不符合SOP且同时态度敷衍。", 1, "严格按适用SOP处理并清晰说明流程。"],
  ["POST_CHALLENGE_CUSTOMER", "质问、反问或直接否定客户", "客服以责备、讽刺或不耐烦方式质问、反问，或未经核实直接否定客户。", 1, "使用礼貌核实语句，避免未经核实直接否定。"],
