...
export function ModelConfigDialog({ models, close, saved }: { models: ModelConfig[]; close: () => void; saved: () => void }) {
  ...
  return <Modal title="模型配置" subtitle="截图提取和字段分析分别使用对应模型，可配置备用模型" close={close}>
    <div className="config-list">{(["vision", "text"] as const).map((purpose) => <section className="model-group" key={purpose}><h3>{purpose === "vision" ? "截图提取模型" : "字段分析模型"} <small>{purpose === "vision" ? "需要支持图片输入" : "结合截图结果和辅助字段"}</small></h3>{models.filter((model) => model.purpose === purpose).map((model) => <div className="config-item" key={model.id}><b>AI</b><span><strong>{model.name}</strong><small>{model.model} · {model.maskedApiKey}</small></span><button onClick={() => test(model.id)}>测试连接</button>{model.isPurposeDefault ? <em>主模型</em> : <button onClick={() => setDefault(model.id)}>设为主模型</button>}<button onClick={() => edit(model)}>编辑</button><button className="danger-text" onClick={() => remove(model.id)}>删除</button></div>)}</section>)}</div>
