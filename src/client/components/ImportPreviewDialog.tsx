...
export function ImportPreviewDialog({
  ...
  const resultConflicts = preview.resultConflicts ?? [];
  return <Modal title="确认导入 Excel" subtitle="导入前检查工作表、表头和嵌入图片" close={onCancel}>
    ...
    {(preview.pendingRecordCount !== undefined || preview.historicalResultCount !== undefined) && <div className="import-preview-section">本次将创建 <strong>{preview.pendingRecordCount ?? 0}</strong> 条待分析记录 · 跳过 <strong>{preview.historicalResultCount ?? 0}</strong> 条完整历史结果</div>}
    ...
    {!!resultConflicts.length && <div className="import-errors"><strong>结果区冲突（{resultConflicts.length} 行）</strong><p>{resultConflicts.map((conflict) => {
      const details = [
        conflict.filledFields.length ? `已填写：${conflict.filledFields.join("、")}` : "",
        conflict.missingFields.length ? `缺失：${conflict.missingFields.join("、")}` : "",
        conflict.extraFields.length ? `多余：${conflict.extraFields.join("、")}` : "",
      ].filter(Boolean).join("；");
      return `${conflict.sheetName} 第 ${conflict.rowNumber} 行${details ? `（${details}）` : ""}`;
    }).join("；")}</p></div>}
    {!preview.imageCount && <div className="notice">未识别到嵌入图片，确认导入后仍会被拒绝。</div>}
    <div className="modal-actions"><button className="button light" onClick={onCancel} disabled={busy}>返回选择板块</button><button className="button dark" onClick={onConfirm} disabled={busy || !preview.imageCount || preview.missingHeaders.length > 0 || Boolean(preview.platformConflicts?.length) || resultConflicts.length > 0}>{busy ? "导入中..." : "确认导入 →"}</button></div>
  </Modal>;
}
