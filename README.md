...
完整文档请从 [文档中心](docs/README.md) 按角色进入。局域网客户端配置见
[局域网其他电脑访问使用手册](docs/guides/lan-client-access.md)；部署与运维见
[局域网部署与单实例运行](docs/guides/lan-deployment.md) 和
[局域网运维手册](docs/guides/lan-operations.md)。
...
2026-09-13 的依赖漏洞定向修复结论保存在
[依赖安全记录](docs/archive/dependency-security-remediation.md)。当前测试数量、依赖审计
和构建状态以最新 CI 质量闸门为准，不沿用历史记录中的固定数字。
...
点击“导出结果”会保留原始工作表、辅助字段和嵌入图片，追加已启用且允许导出的结果字段。解析状态、复核状态和复核备注仅保留在系统界面与数据库中，不写入业务导出文件；知识匹配内部字段也不会写入导出文件。
